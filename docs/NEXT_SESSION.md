...
**Cloud incident (must not repeat):** A `podStop` + `podResume` cycle on RunPod re-provisioned the pod onto a fresh container with empty `/workspace`. The pod had Container Disk only (no Network Volume), so the persistent volume didn't follow. What was lost: the 104 GB human_v8_100k memmap, the code clone, the venv, and any mid-epoch in-flight state. **What was NOT lost: per-epoch optimizer + scheduler state** — that's serialized into every saved `epoch_NNN.pt` (verified: keys include `optimizer_state_dict`, `scheduler_state_dict`), and we scp'd e1/e2/e3 down to local after each epoch. A re-bootstrap (memmap re-download from R2 ~25 min, code re-clone ~5 min) followed by `python train_bc.py --resume epoch_003.pt ...` (no `--lr-restart`) would have continued e4 with AdamW momentum and scheduler position fully preserved. Decided to terminate and lock in e3 rather than re-bootstrap, but the recovery cost was modest (~30 min wall-clock + ~5 hr per epoch of A100 time), not the catastrophic state-loss originally framed. The original Session 48 `podResume` had also failed SSH key injection — `~/.ssh/authorized_keys` wasn't repopulated, so we fell back to web terminal, then attempted a stop-start cycle to re-trigger the startup script, which is when the volume vanished. **Future cloud runs MUST attach a Network Volume mounted at `/workspace`** (RunPod console → Storage → Network Volume). With Container Disk, pause/resume usually works but stop/start can land on a different host. Pod `t56a2jndi8iyz8` terminated; we are not billing.
