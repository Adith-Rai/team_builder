...
| Config | Collect | Update | Total | Δ vs pre-arc legacy |
|---|---|---|---|---|
| Legacy mb=16 (pre-arc baseline) | 573s | 1865s | 2438s | — |
| Packed mb=16 (Phase B alone) | 573s | 1729s | 2302s | -7.3% |
| Packed mb=32 (initial 2a ship) | 578s | 1054s | 1632s | -43.5% |
| Packed mb=48 (bf16 sweep) | 560s | 660s | 1220s | -64.6% |
| **Packed mb=64 (final 2a ship)** | **550s** | **551s** | **1101s** | **-70.5% / 3.38×** |

- Overall wall: 2438s → 1101s = **-55%** at packed mb=64
- 200-iter Phase 2: ~5.7 days / ~$200 → **~2.5 days / ~$90** (saves ~80 hr / ~$110 per run)
- Numerical drift: within bf16 noise across all mb (pi/v/ent/kl/bc_kl consistent)
...
**S65 — ARCH IMPASSE (2026-05-17/18)**: all cheap tuning levers for pool-growth slowdown exhausted. **PFSP allocator polish SHIPPED** on `perf/pfsp-allocator-polish` (opt-in `--pfsp-allocator balanced`, 15/15 unit tests, prod A/B unrun — modest 7-10% at 15w, 25-37% at 8w with small pools). **15w retest PASSED** — task #21 did not reproduce; CIS Phase 4.6 recovery handles partial stalls. **REFUTED**: battle server within-run degradation (Run B warm-BS iter 0 = 568s vs Run A fresh-BS = 555s, 13s noise); CIS adaptive batching Stage A (every iter +14-37% slower due to per-fire fixed overhead × increased fire frequency); CPU opp inference service (344 inf/s aggregate at 32 workers vs 2300 GPU baseline — fundamentally not competitive at 20M transformer). User: "phase 2 is not ready... back to the drawing board." Remaining options ruled out: multi-GPU (budget), pool-size cap (hinders training, user-rejected), cross-slot GPU batching (PyTorch limit on different models per forward). **Next session: research task** — study Metamon (in `metamon_ref/`), OpenAI Five, AlphaStar, ps-ppo, VGC-Bench for how PFSP/self-play implementations handle multi-opp inference scaling. Full session details in `memory/project_s65_arch_impasse.md`.
...
| **S64 post-wrap 2a SHIPPED at mb=64 (CURRENT)** | S64 | `--tier3-minibatch-size 64` (was 16; initial 2a ship was mb=32, corrected after fp32-smoke-vs-bf16-prod defect surfaced). Full bf16 sweep at clean-state prod: mb=16→1729s, mb=32→1054s, mb=48→660s, mb=64→**551s** update wall. **Cumulative -70.5% / 3.38× vs pre-arc legacy mb=16.** Diminishing returns past mb=64 (update ≈ collect, collect-bound takes over). Key learning: fp32 smoke profiles do NOT predict bf16 prod kernel selection. | `memory/project_s64_phase_b_results.md` §3.5 |
