...
1. **Read this file** (project_session_boot_protocol.md) — standing orders
2. **Read `feedback_engineering_standard.md`** — quality standard (no shortcuts, ship correctly first time, read code + docs before touching)
3. **Read `project_strategic_frame.md`** — locked sequencing (vertical → horizontal, gen-9-as-infra-testbed, multi-gen-AGNOSTIC vs SPECIFIC)
4. **Read `project_cis_4_6_design.md`** — most recent shipped infra (Phase 4.6 unified recovery)
5. **Read `next-prompt.txt`** at project root — session-specific deep state (current iter, in-flight work, deferred items)
6. **Read `docs/PHASE1_V3_OBSERVATIONS.md`** — Phase 2 design implications + iter 39/49 EVAL diagnosis
7. **Run production status check** — first ACTION (SSH command in next-prompt §E). Report what's running.

If any of these files conflict, surface the conflict. Don't silently reconcile.
...
4. **Phase 1 v3 final eval + analysis** — pull final.pt, run eval suite + H2H gauntlet, document final smart_avg + Elo + per-bot.

5. **C6 — Tier 3 20-iter A/B validation** — runs against Phase 1 v3 baseline, on dev pod, ~$10-15.

6. **Easy items JIT for Phase 2 launch** — perm/canonical fix, lr scheduling, source 100-150 elite teams, bf16 dev pod smoke. Done immediately before Phase 2 launches with shapes/code stable post-Tier-3.

7. **Phase 2 launch** — `--cis --pipeline --bf16` + Tier 3 + train_step compile + team mix (50-70% procedural / 30-50% NEW elite teams) + perm fix + external opps (mcts-fast + Kakuna/SmallRL/Minikazam). Init from Phase 1 v3 final.pt. Target ~10-15 min/iter, ~$200-300 for 200 iters. **Where elite gen 9 gets measured.**

8. **Post-Phase-2 evaluation** — H2H gauntlet vs published bots tells us if we hit elite. If yes: proceed to multi-gen prep. If no: investigate + iterate (likely lr re-ablation post-Tier-3, or further wave training).
...
## §8. Plan trajectory — in flight + deferred
...
- **H2H gauntlet wiring** — 3-4 hrs, needed for Phase 1 v3 final eval
- **C5 train_step compile** — bundled with C4
- **train_rl.py wiring `--tier3` flag** — connects new path to production
- **Dev pod end-to-end smoke** — validates the full stack at small scale

### Post-Phase-1-v3

- **Pull final.pt + full eval suite** including H2H gauntlet
- **C6 — 20-iter A/B validation** vs Phase 1 v3 baseline
- **Easy items JIT for Phase 2**: perm/canonical fix, lr scheduling impl, source 100-150 real elite teams, bf16 dev pod smoke

### Phase 2 launch

- `--cis --pipeline --bf16` + Phase 4.4-4.6 + Tier 3 + train_step compile
- Team mix: 50-70% procedural / 30-50% NEW real elite teams (NOT the 16 mm-competitive eval set)
- Perm at eval enabled
- External opps in PFSP pool (mcts-fast + Kakuna + SmallRL + Minikazam)
- lr scheduling (likely cosine decay or step decay; re-ablate post-Tier-3)
- Absolute early-stop threshold
...
**Recent infra (Phase 4.x CIS)**:
- `project_cis_4_3_design.md` — pool-mirror multi-slot
- `project_cis_4_6_design.md` — Option B unified recovery
...
- `STATUS.md` — historical era analysis (Elo trajectory)
